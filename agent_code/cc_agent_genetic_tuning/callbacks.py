...
def setup(self):
    ...
    self.feature_dict = get_feature_dict()
    if self.train and not os.path.isfile("model_running/my-saved-model.pt"):
        print("First round")
        self.logger.info("Setting up model from scratch.")
        self.first_training_round = True

        if os.path.isfile("model_creation/initial_model.pt"):
            with open("model_creation/initial_model.pt", "rb") as file:
                self.model = pickle.load(file)

        else:
            raise Warning("No initial model found.")

    elif self.train and os.path.isfile("model_running/my-saved-model.pt"):
        print("Second round")
        self.logger.info("Loading model from saved state.")
        self.first_training_round = False
        with open("model_running/my-saved-model.pt", "rb") as file:
            self.model = pickle.load(file)
