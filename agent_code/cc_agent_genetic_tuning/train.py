...
def TD_nstep(self, n):
    ...
    if len(transitions) == n:
        ...
        self.logger.debug(
            f"ChangedValue by {alpha * ( G - self.model[update_index, update_action])}"
        )
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    with open("model_running/my-saved-model.pt", "wb") as file:
...
def save_game_statistic(self, game_state):
    filename = "model_testing/stat.json"
    ...
    if round == 100:
        ...
        #         stats_dict.setdefault(len(stats_dict.keys()),self.score)
        # else:
        stats_dict = {"0": self.score}
        print(self.score)
